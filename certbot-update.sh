...
for cert_dir in $certs_directory*; do
    ...
    # 将过期日期转换为本地时间并格式化为中文日期格式
    expiration_date_local=$(date -d "${expiration_date}" +"%Y年%m月%d日 %H时%M分%S秒")

    # 输出证书过期日期（中文格式）
    echo "过期日期： ${expiration_date_local}"
    ...
    if [ $days_until_expiry -le $days_before_expiry ]; then
        ...
        # 续签证书
        certbot certonly --standalone -d $domain --email 12345678@qq.com --agree-tos --no-eff-email --force-renewal
